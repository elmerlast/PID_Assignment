...
if (isset($_POST["btnRegister"])) {
	...
	$sql = "select m_email from tbl_users;";
	$result = mysqli_query($link, $sql);
	while($row = mysqli_fetch_row($result)){
		$rows[]=$row;
	}
				...
				<div class="card">
					<header class="card-header">
						<div class="row" style="position: relative;">
							<div class="col-6">
								<h4 class="card-title mt-2">註冊會員</h4>
							</div>
							...
							<div class="form-row">
								...
								<div class="col form-group">
									...
									<input type="email" id="inputEmail" name="inputEmail" class="form-control"
										placeholder=""
										value="<?php if(isset($_POST["inputEmail"])) { echo "{$_POST["inputEmail"]}"; } ?>">
							...
							<div class="form-row">
								<div class="col form-group">
									...
									<input type="tel" class="form-control" id="inputPhoneNumber" name="inputPhoneNumber"
										value="<?php if(isset($_POST["inputPhoneNumber"])) { echo "{$_POST["inputPhoneNumber"]}"; } ?>"
										required pattern="[0-9]{9,10}">
								</div> <!-- form-group end.// -->
								<div class="col form-group">
									<label>生日</label>
									<input class="form-control" type="date" id="inputBirthday" name="inputBirthday"
										value="<?php if(isset($_POST["inputBirthday"])) { echo "{$_POST["inputBirthday"]}"; } ?>">
								</div> <!-- form-group end.// -->
							</div> <!-- form-row end.// -->
					...
					<div class="border-top card-body text-center">
						己經有帳號了嗎 ? <a href="/PID_Assignment/member/login.php">現在就登入！</a>
					</div>
					<a class="nav-link" href="/PID_Assignment/index.php">
						點此返回CC音饗<span class="fa fa-arrow-left"></span>
					</a>
				</div> <!-- card.// -->
			</div> <!-- col.//-->
